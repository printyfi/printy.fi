...
export default function EnhancedTable({ claimable, crv, ibEUR, rKP3R }) {
  ...
  const onClaim = (asset) => {
    ...
    } else if (asset.type === 'Printy' && asset.description === 'Fee Claim') {
      stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_CLAIM_DISTRIBUTION_REWARD, content: {} })
    } else if (asset.type === 'Printy' && asset.description === 'Vesting Rewards') {
      stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_CLAIM_VESTING_REWARD, content: {} })
    } else if (asset.type === 'Printy' && asset.description === 'Redeemable KP3R') {
                        ...
                        <Grid item lg={12} md={12} sm={12} xs={12} className={classes.endAsset}>
                          <Grid container spacing={0}>
                            <Grid item lg={9} md={9} sm={9} xs={9}>
                              ...
                              <Typography variant="h5" className={classes.textSpaced} color="textSecondary">
                                ${row.symbol === 'CRV' && formatCurrency(BigNumber(row?.earned).times(crv?.price))}
                                {row.symbol === 'ibEUR' && formatCurrency(BigNumber(row?.earned).times(ibEUR?.price))}
                                {(row.symbol === 'rKP3R' || row.symbol === 'kp3R') &&
                                  formatCurrency(BigNumber(row?.earned).times(rKP3R?.price))}
                                {!['CRV', 'ibEUR', 'rKP3R', 'kp3R'].includes(row.symbol) && formatCurrency(0)}
                              </Typography>
                            </Grid>
                            ...
                            <Grid item lg={3} md={3} sm={3} xs={3} className={classes.alignR}>
                              ...
                              >
                                <Typography className={classes.actionButtonText}>
                                  {claimLoading && (!claimedAsset || claimedAsset.symbol === row.symbol)
                                    ? `Claiming`
                                    : `Claim`}
                                </Typography>
                                {claimLoading && (!claimedAsset || claimedAsset.symbol === row.symbol) && (
